...
const GiftDetail = () => {
  ...
  const { user, profile, deductFromBalance, refreshProfile } = useAuth();
  ...
  const handlePurchase = async () => {
    ...
    setPurchasing(true);
    try {
      console.log(`🎁 Starting gift purchase: ${gift.title} for $${totalAmount}`);
      ...
      if (result.success) {
        console.log(`✅ Gift purchase successful: ${result.orderNumber}`);
        
        // Update balance in UI
        deductFromBalance(totalAmount);
        
        toast.success(`Gift order placed successfully! Order #${result.orderNumber}`);
        
        // Navigate to order confirmation or tracking
        navigate(`/gift-tracking/${result.trackingCode}`);
      } else {
        console.error(`❌ Gift purchase failed: ${result.error}`);
        toast.error(result.error || "Failed to place gift order");
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.success && result.orderId) {
        ...
        if (paymentResult.success) {
          ...
          toast.success(`Gift order placed and paid successfully! Order #${result.orderNumber}`);
